...
def transfer_item(item_id, obra_id_destino, destino_nome, quantidade, observacao, responsavel):
    """
    Transfere 'quantidade' do item para outra obra.
    Se a obra destino já possui o mesmo item (mesmo nome + categoria), soma; senão cria um novo registro.
    Também registra as movimentações de Saída e Entrada com origem/destino nas observações.
    """
    conn = get_db_connection()
    try:
        cur = conn.cursor()

        # 1) Busca o item original
        cur.execute("""
            SELECT nome, categoria_id, qtd_estoque, qtd_em_uso, condicao, status
        ...
        # 2) Atualiza obra de origem (decrementa estoque)
        ...
        # 3) Verifica/atualiza item na obra destino
        cur.execute("""
        ...
        # 4) Registra movimentações (sempre fora do if/else!)
